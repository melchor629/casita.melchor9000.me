...
import { useCallback, useEffect, useState, type InputEvent } from 'react'
...
export default function Vocabulary({ changePage }: { readonly changePage: () => void }) {
  ...
  return (
    <>
      <div className="flex gap-2 max-w-3xl w-full pt-4 px-4 pb-4 bg-primary-bg/30 backdrop-blur-sm z-10">
        <input
          type="text"
          id="vocab-filter"
          className={clsx(
          ...
          value={inputValue}
          onInput={useCallback((ev: InputEvent<HTMLInputElement>) => {
